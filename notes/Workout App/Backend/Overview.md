...
- **The server refuses a snapshot whose schema version it doesn't know**, the same discipline that makes `liftimport` refuse a database predating `v13`.
...
Not yet done in 2.1: the upload triggers, migration `v14_cognitoSub`, Cognito itself, and `server/infra/`.
