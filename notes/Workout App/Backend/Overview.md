...
**The phone writes to S3 itself.** Temporary credentials from a Cognito identity pool, whose authenticated role is scoped by a principal tag carrying the user pool `sub` to exactly `users/${sub}/*`. There is no API in front of the bucket: nothing mints a URL and no server code authorises a request, because IAM does it before any code runs. `server/INFRA-SPEC.md` is the resource-level spec.

- **This replaced a presigning API** — five routes, six Lambdas and a device lease — and the trade is worth recording rather than quietly making. What went: an HTTP surface, most of `server/`, and two claims examined below. What arrived: one Lambda, one table, and a phone that needs no round trip to start an upload.
- **The one identifier trap.** The obvious IAM variable, `${cognito-identity.amazonaws.com:sub}`, is the *identity pool's* identity id, not the user pool `sub` — and the `sub` is what `v14_cognitoSub`, the S3 prefix and every DynamoDB item already key on. Mapping the claim to a principal tag (`use_defaults = false`) is what keeps one lifter to one identifier; getting it wrong means two, and a mapping table to keep them married.

**The record is derived from the object, never reported by the uploader.** `index_snapshot` runs on `ObjectCreated`, downloads the snapshot, opens it, and reads `grdb_migrations` and the row counts itself.

- **This is what replaced a signature, and it's stronger.** The presigning design baked the schema version into SigV4 so S3 would reject a mismatched header — genuinely enforceable, and it found a real bug in the making (boto3 presigns **SigV2** by default, which carries metadata as *query parameters* rather than signed headers; measured, not assumed). But IAM has no condition key for `x-amz-meta-*`, so under direct credentials the stamp is self-reported — and the answer is not to reach for a weaker signature, it's to open the file. A signature proved the phone said the same thing twice. Reading proves what the bytes contain. There is one user and no adversary here: the risk was never a lying phone, it was a phone that computed its own stamp wrongly, which a signature could never have caught.
- **It's `ExerciseStatsStore`'s discipline, pointed at the bucket.** One writer, rebuild-only, derived. The index can be *stale*; it cannot disagree with the object, and re-reading the bucket is always a valid repair.
- **`KNOWN_MIGRATIONS` stopped being documentary.** `grdb_migrations` is an unordered set, so "the last applied migration" only exists relative to a registration order — the Python list *is* that order, and sorting it alphabetically (where `v9` follows `v14`) would report every snapshot at the wrong version. Still pinned to `Migrations.swift` by a test that parses the real file.
- **Reads are pinned to the event's version id.** On a versioned bucket a bare read returns whatever is current, so two uploads a minute apart — a finished workout and a plan save — would have the first event read the second object's contents and file them under the first one's etag and size. Internally inconsistent, with nothing to indicate it.
- **There is still no commit call.** The object *is* the trigger, so a phone that dies between the PUT and a report leaves nothing to reconcile.
- **The digest is still enforced.** The phone sends `x-amz-checksum-sha256` with its own PUT and S3 verifies the body against it, so a corrupted upload stays impossible rather than detectable-later. That property survived the redesign intact; it never depended on who signed the request.
- **An unopenable file is recorded, not retried.** A truncated upload will still be truncated on the retry, and an empty index reads as "nothing was uploaded" rather than "something wrong was." Transient failures do escape, so S3 retries those.

**The device lease is gone; what it was for is not.** With standing credentials there's no chokepoint at which a lease could be enforced, and no cleverness recovers one — a pre-token-generation trigger could bake lease state into a claim, but credentials live an hour, so enforcement would lag by up to an hour. **A conditional `PutObject` replaces it and is better.** `If-Match` on the etag of the snapshot this phone last uploaded means a **412 is S3 telling the phone another device wrote since** — enforced rather than believed, costing no Lambda, no table and no Terraform, and reusing the watermark `UserDefaultsWatermarkStore` already keeps.

- **It's detection of a lost update, not mutual exclusion.** It doesn't stop the second phone, it tells it, and the lifter decides whether to overwrite — Core Tenet 1 in exactly the place it belongs, rather than the app silently picking a winner. It also catches the same phone running on a stale watermark, which a lease never would have.
- **Object Lock is not this**, and was considered. It's WORM retention for compliance regimes: it stops a specific object *version* being deleted, and since a PUT to an existing key creates a *new* version, the second phone's upload succeeds regardless. It carries no notion of who holds anything, it would stop the lifecycle rule expiring noncurrent versions (so storage grows forever), and it can only be enabled at bucket creation — an irreversible answer to a question it doesn't address.
- **Even a bare last-writer-wins would have been acceptable**, because the phone is the system of record. The snapshot is a backup: two phones alternating uploads means the last wins and *nothing is destroyed*, since the losing phone still holds its own log in full. The one genuinely destructive operation is restore, and that decision lives on the device in `SnapshotImporter`, which refuses a snapshot missing local workouts.
- **The schema gate went with the lease, and lost nothing.** Its floor was `v14_cognitoSub`, and a build without that migration has no column to record an account — so it can't sign in, so it can't upload. The floor was unreachable by arithmetic and the gate's entire runtime behaviour was to record a version, which is now done by reading it.
...
1. New top-level directory in `terraform-infrastructure` (`lift-coach/`), own `main.tf`/`variables.tf`/`outputs.tf`/`modules/`
2. Own S3 backend state key: `lift-coach/terraform.tfstate` (same bucket/lock table as `amazing-adventure`, isolated key)
