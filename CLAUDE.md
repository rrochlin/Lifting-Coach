...
- `server/` — the phase 2 server, a Python (uv) project holding the Lambda handlers. **It reads the phone's snapshot and never writes it**; `server/README.md` states the contract and is worth reading before touching any of it. `cd server && uv run pytest` needs no AWS account — every AWS surface is behind a narrow protocol with an in-memory implementation.
...
- **Profile: data export.** Import is deliberately not going here (see the `scripts/` paragraph above); export still has no home. The screen is themed and hosts the unit switcher; its data section is an honest "NOT STARTED".
