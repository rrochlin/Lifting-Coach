...
Built: `WorkoutSession` + `WorkoutStore` + tracker UI; `PlanStore` + `UserStore` + planner UI; homepage metrics; theme; catalog import + program loading; the `scripts/` import pipeline; History editing. **301 Swift tests + 53 Python tests.**
...
**The planner authors sets in bulk: `ALL 4 × 5 · 77.5 % → APPLY`.** `PlannedWorkoutDraft.setWorkingSets` (model package, tested) + `BulkSetRow` under each exercise header. A program is written as "4×5 @ 225" and authoring it a row at a time was four taps of Add Set and twelve fields; the block overview has always *read* a plan back this way (`setGroups`), and this is the same shape going in.
- **Working sets only.** Warmups and drops keep their positions — a bulk prescription is never an instruction to delete a ramp.
- **Resizes rather than replaces**, so surviving sets keep their identity and their own per-set effort and rest; new ones inherit both from the first working set.
- **Seeded from the sets that already exist**, so it reads as the current prescription rather than as blanks, and it carries the load *mode* they're written in (changing the mode is still the per-set menu's job — offering it twice is how two controls drift apart). The per-set rows stay authoritative for anything non-uniform: back-offs, ramps, a per-set RPE.
