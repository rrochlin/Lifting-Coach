...
**Both registered phones are dev devices and their data is expendable** — the iPhone 13 Pro Max and the iPhone 15 Pro Max, the owner's explicit call on each. (This file said "the iPhone 13" for a long time, written as though there were one phone; the 15 was registered too and every migration decision made under that sentence had a device it didn't cover.) That relaxes a constraint this file used to state in the strongest terms: `eraseDatabaseOnSchemaChange` (DEBUG-only) wiping the phone was treated as a disaster, so `v2_exerciseCatalog` through `v7_exerciseSuggestions` are all strictly additive. It isn't a disaster any more, and `v8_dropMatchedSlug` is deliberately non-additive. `v9_userPreferredUnit` is back to additive (a column with a default).
