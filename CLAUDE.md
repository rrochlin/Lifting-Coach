...
- **The clock itself is editable.** Tapping it opens `RestAdjuster` — the same editor the per-set rest chip uses, titled "rest remaining" — with presets and ±15/±30, backed by `RestTimer.setRemaining` / `TrackerModel.setRestRemaining`. ±30 is fine for "one more breath"; "make it three minutes" took six taps and arithmetic. One idea, one editor. `startedAt` stays put so the progress bar still measures this rest from when it began. The popover is anchored **outside** the row's `TimelineView`, which rebuilds every second — a presentation hung off that would dismiss itself under the lifter.
