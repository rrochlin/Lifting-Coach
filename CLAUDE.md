...
Built: `WorkoutSession` + `WorkoutStore` + tracker UI; `PlanStore` + `UserStore` + planner UI; homepage metrics; theme; catalog import + program loading; the `scripts/` import pipeline; History editing. **295 Swift tests + 53 Python tests.**
...
**Rest makes a sound, and `RestChime` is why it's not the notification's job.** The scheduled `UNNotification` does carry `.sound` — but it needs a permission the lifter may have declined, and it's the fallback for when the app *isn't* on screen, which is the edge case rather than the main one. The chime plays through the **silent switch** (`.playback`), because a gym is full of phones on silent and a rest timer nobody can hear doesn't work; and it **refuses to stop the music** (`.mixWithOthers` + `.duckOthers`), because an app that pauses your playlist every two minutes gets its sound turned off. The session is configured lazily at the first rest period rather than at launch — activating one is a claim on the device's audio, and an app that makes it on startup appears in the now-playing controls before it has made a sound.
