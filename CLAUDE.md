...
- **It's two views, not one — `RestReadout` and `RestEditor`.** The line is the clock; the editor is everything you'd touch to change it, and it lives *under* the line as its own component. That split is what keeps the clock still. When both were one view, opening the editor relaid the whole thing out and the readout's pieces — icon, label, rule, digits — each drifted to their new positions at their own rate, so the clock appeared to swim through the controls arriving underneath it. The readout is a `geometryGroup()` now: one rigid unit that either moves or doesn't.
- **The expand is not animated, deliberately.** This control lives in a `List` row, and animating the row's height lays the cell's contents out for the open state inside a frame that's still growing — the clock and the steppers drift toward each other and cross before settling. Two transitions were tried (a slide from the top edge, then a scale from the top anchor) and both showed it; the artifact is the row resize, not the transition. Verified by recording the simulator and stepping frames: the change now happens between two frames with nothing in between. The one thing that still animates is the caret's rotation, which is a transform and can't drag layout with it.
- **Collapsed until asked.** All that shows is `REST ——— 2:00`; a running rest just runs. Tapping opens ±30/±15, the presets, and the one action that applies. Expiry opens it once — a REST COMPLETE readout with no way to acknowledge it is a dead end — and that's a **latch, not a derived value**, because the lifter has to be able to close it again.
- **A running clock shows nothing but the clock.** Rest proceeds unmolested: while it counts down there are no steppers and no presets, just the time and the track draining under it — two thin rows. Tapping opens the editing interface (the same rows the `.prescription` state shows, since they're the same control) and tapping again puts them away. Everything is still inline; it's simply not all on screen at once, and there's no longer a presentation hanging off a view that rebuilds every second.
...
- **The `EditButton` is gone**, and with it the `\.editMode` binding. `.onMove` is still on both the exercise and set `ForEach`es, so reordering should be a long-press drag; nothing here can tap, so **that is unverified** — check it by hand before trusting it, since `notes/Feedback.md` asks for drag-to-reorder explicitly. If long-press turns out not to work outside edit mode, the fix is a reorder affordance in the exercise's `…` menu, not the button back in the toolbar.
