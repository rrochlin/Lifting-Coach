...
- Edited through `RestPicker` (`Sources/App/Components/`), a chip styled like the reps/weight fields that opens `RestControl` — see below. It's shared: the tracker writes `restOverride`, the planner writes `PlannedSet.restTime` per set. The planner's exercise-level `RestMenu` still writes every set at once, which is the common case.
...
**Rest is one control in two states — `RestControl` (`Sources/App/Components/`).** `.prescription` is a rest value being set; `.running` is that value counting down. Same skeleton in both — clock, track, ±30/±15, presets, one action — and only what has to differ does: the running state moves, fills its track, and turns amber (cyan the instant it's up). There used to be two components with two layouts and two ideas about where the clock goes, which meant learning a second control mid-workout, at the moment there's least attention to spare. The per-set chip (`RestPicker`) opens it in `.prescription`; the tracker renders it in `.running` under the set that started the rest; the planner's chips open the same thing.
- **Everything is inline, in both states.** The running control carries its own presets rather than opening a sheet to reach them — "make it three minutes" is one tap, and there's no longer a presentation hanging off a view that rebuilds every second.
